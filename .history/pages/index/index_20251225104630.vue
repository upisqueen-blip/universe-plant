	...
	export default {
		...
		methods: {
			...
			detectEnvironment() {
				...
				try {
					// 检查是否在小程序环境中
					if (typeof wx !== 'undefined' || (typeof uni !== 'undefined' && uni.getSystemInfoSync)) {
						...
						if (systemInfo.platform === 'devtools' || 
							...
							typeof wx !== 'undefined') {
							this.environment = 'mp';
							console.log('运行时检测：小程序环境');
							return;
						}
					}
				} catch (e) {
					console.log('检测环境时出错:', e);
				}
				
				// 默认为浏览器环境
				this.environment = 'browser';
				console.log('运行时检测：浏览器环境');
			},
			
			// 初始化MQTT连接
			initMQTT() {
				if (this.environment === 'mp') {
					// 小程序环境使用WebSocket实现MQTT协议
					this.initMQTTForMP();
				} else {
					// 浏览器环境使用mqtt.js库
			...
			initMQTTForMP() {
				try {
					...
					console.log('小程序环境尝试连接MQTT服务器:', wsUrl);
					...
					// 监听WebSocket连接打开事件
					uni.onSocketOpen(() => {
						console.log('小程序WebSocket连接成功');
						this.isOnline = true;
						this.showConnectionStatus = true;
						this.connectionMessage = 'MQTT服务器连接成功';
						
						// 连接成功后发送MQTT CONNECT包
			...
			buildMQTTConnectPacket(clientId, username, password) {
				// 简化的MQTT CONNECT包构建
				// 注意：这是一个简化版本，实际MQTT协议更复杂
				const packet = [];
				
				// 固定头
				packet.push(0x10); // CONNECT命令
				
				// 可变头
				packet.push(0x00); // 协议名长度MSB
				packet.push(0x06); // 协议名长度LSB
				packet.push('M'.charCodeAt(0));
				packet.push('Q'.charCodeAt(0));
				packet.push('I'.charCodeAt(0));
				packet.push('s'.charCodeAt(0));
				packet.push('d'.charCodeAt(0));
				packet.push('p'.charCodeAt(0));
				
				packet.push(0x04); // 协议版本
				...
				// 保活时间
				packet.push(0x00); // 保活时间MSB
				packet.push(0x3C); // 保活时间LSB (60秒)
				
				// 客户端标识符
				const clientIdBytes = this.stringToBytes(clientId);
				packet.push(clientIdBytes.length >> 8); // 长度MSB
				packet.push(clientIdBytes.length & 0xFF); // 长度LSB
				for (let i = 0; i < clientIdBytes.length; i++) {
					packet.push(clientIdBytes[i]);
				}
				
				// 用户名
				if (username) {
					const usernameBytes = this.stringToBytes(username);
					packet.push(usernameBytes.length >> 8); // 长度MSB
					packet.push(usernameBytes.length & 0xFF); // 长度LSB
					for (let i = 0; i < usernameBytes.length; i++) {
						packet.push(usernameBytes[i]);
					}
				}
				...
				if (password) {
					...
					packet.push(passwordBytes.length >> 8); // 长度MSB
					packet.push(passwordBytes.length & 0xFF); // 长度LSB
					for (let i = 0; i < passwordBytes.length; i++) {
						packet.push(passwordBytes[i]);
					}
				}
			...
			sendInitCommand() {
				...
				if (this.environment === 'mp') {
					// 小程序环境使用WebSocket发送MQTT PUBLISH包
					this.sendMQTTPublish('mqttsub', JSON.stringify(command));
				} else {
					// 浏览器环境使用mqtt.js库
			...
			// 发送MQTT PUBLISH包（小程序环境）
			sendMQTTPublish(topic, payload) {
				if (this.environment !== 'mp') return;
				
				// 构建MQTT PUBLISH包
				const publishPacket = this.buildMQTTPublishPacket(topic, payload);
				...
				console.log(`发送消息到主题 ${topic}:`, payload);
			},
			
			// 构建MQTT PUBLISH包
			buildMQTTPublishPacket(topic, payload) {
				const packet = [];
				
				// 固定头
				let flags = 0x30; // PUBLISH命令
				flags |= 0x01; // QoS 1
				packet.push(flags);
