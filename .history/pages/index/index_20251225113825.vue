	...
	export default {
		...
		methods: {
			...
			detectEnvironment() {
				...
				try {
					// 检查是否在小程序环境中
					// 浏览器环境中没有wx对象，这是最可靠的判断标准
					if (typeof wx !== 'undefined') {
						this.environment = 'mp';
						console.log('运行时检测：小程序环境（通过wx对象检测）');
						return;
					}
					
					// 额外检查：如果是浏览器环境，确保不会误判
					if (typeof window !== 'undefined' && typeof document !== 'undefined') {
						this.environment = 'browser';
						console.log('运行时检测：浏览器环境（通过window和document对象检测）');
						return;
					}
					
					// 最后的备选检测：检查uni对象的平台信息
					if (typeof uni !== 'undefined' && uni.getSystemInfoSync) {
						...
						if (systemInfo.platform === 'devtools' || 
							...
							this.environment = 'mp';
							console.log('运行时检测：小程序环境（通过平台信息检测）');
							return;
						}
					}
				} catch (e) {
